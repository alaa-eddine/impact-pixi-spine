...
PIXI.BitmapText.prototype.updateText = function()
{
    ...
    {
        var char = PIXI.Sprite.fromFrame(this.fontName+chars[i].charCode); // bugfix
...
PIXI.SpineLoader.prototype.onJSONLoaded = function()
{
	if (this.ajaxRequest.readyState == 4)
	{
		 if (this.ajaxRequest.status == 200 || window.location.href.indexOf("http") == -1)
	 	{
			...
			var data = this.ajaxRequest.responseText;

			PIXI.AnimCache[this.url] = data;
    ...
    if (this.ajaxRequest.readyState == 4)
    {
        if (this.ajaxRequest.status == 200 || window.location.href.indexOf("http") == -1)
        {
            ...
            for (var i = 0; i < letters.length; i++)
            {
                ...
                PIXI.TextureCache[data.font+charCode] = new PIXI.Texture(this.texture, textureRect); // bugfix
...
ig.System.inject({
	init: function( canvasId, fps, width, height, scale ) {
		...
		// Pixi.JS
		this.renderer = new PIXI.autoDetectRenderer(width, height, ig.$(canvasId));
		...
		if(this.initMobile) this.initMobile();
	},
...
// 	addAnim: function( name, speed, frames, stop ) {
// 		if( !this.animSheet ) {
...
// 				anim.frames.push(PIXI.Texture.fromFrame(frames[i]));
// 			};
...
// 		if( !this.currentAnim ) {
// 			this.currentAnim = new PIXI.MovieClip(anim.frames);
// 			this.currentAnim.animationSpeed = anim.speed;
// 			this.currentAnim.play();
// 			this.currentAnim.position.x = this.pos.x - this.offset.x - ig.game._rscreen.x;
// 			this.currentAnim.position.y = this.pos.y - this.offset.y - ig.game._rscreen.y;
// 			this.currentAnim.update = function(){};
// 			this.currentAnim.draw = function(){};
// 			if(this.container) {
// 				var cont = eval("ig.game."+this.container);
// 				cont.addChild(this.currentAnim);
// 			}
// 			else ig.game.entityContainer.addChild(this.currentAnim);
// 		}
// 	},

// 	setAnim: function(anim) {
// 		this.currentAnim.textures = anim.frames;
// 		this.currentAnim.animationSpeed = anim.speed;
// 	},

// 	update: function() {
// 		if(this.currentAnim) {
// 			this.currentAnim.position.x = (this.pos.x + this.currentAnim.anchor.x * this.size.x) - this.offset.x - ig.game._rscreen.x;
// 			this.currentAnim.position.y = (this.pos.y + this.currentAnim.anchor.y * this.size.y) - this.offset.y - ig.game._rscreen.y;

// 			this.currentAnim.scale.x = this.flip ? -Math.abs(this.currentAnim.scale.x) : Math.abs(this.currentAnim.scale.x);
// 		}
// 		this.parent();
// 	},

// 	kill: function() {
// 		if(this.currentAnim) ig.game.entityContainer.removeChild(this.currentAnim);
// 		this.parent();
// 	}
// });
...
ig.Entity.inject({
	...
	init: function(x,y,settings) {
		...
		if(this.image) {
			this.sprite = new PIXI.Sprite(PIXI.Texture.fromFrame(this.image.path));
		}

		if(this.frames.length > 0) {
			for (var i = 0; i < this.frames.length; i++) {
				this.frames[i] = PIXI.Texture.fromFrame(this.frames[i]);
			};
		...
		if(this.sprite) {
			...
			this.sprite.anchor.x = this.sprite.anchor.y = this.anchor;
			ig.game.entityContainer.addChild(this.sprite);
		}

		if(this.interactive && this.sprite) {
	...
	update: function() {
		if(this.sprite) {
			this.sprite.position.x = (this.pos.x + this.sprite.anchor.x * this.size.x) - this.offset.x - ig.game._rscreen.x;
			this.sprite.position.y = (this.pos.y + this.sprite.anchor.y * this.size.y) - this.offset.y - ig.game._rscreen.y;
	...
	kill: function() {
		if(this.sprite) ig.game.entityContainer.removeChild(this.sprite);
		this.parent();
	}
});

// PIXIText = ig.Entity.extend({
// 	text: null,
// 	anchor: 0.5,

// 	init: function(x,y,settings) {
// 		this.sprite = new PIXI.Text(settings.text, settings.textSettings);
// 		this.offset.x = this.sprite.width / 2;
// 		this.offset.y = this.sprite.height / 2;
// 		this.parent(x,y,settings);
// 	},
...
ig.Game.inject({
	...
	init: function() {
		...
		if(this.interactive) {
			ig.system.stage.mousemove = ig.system.stage.touchmove = this.mousemove.bind(this);
			ig.system.stage.click = ig.system.stage.tap = this.click.bind(this);
			ig.system.stage.mousedown = ig.system.stage.touchstart = this.mousedown.bind(this);
			ig.system.stage.mouseup = ig.system.stage.mouseupoutside = ig.system.stage.touchend = ig.system.stage.touchendoutside = this.mouseup.bind(this);
		}

		if(ig.ua.mobile) {
			var canvas = document.createElement("canvas");
			canvas.width = ig.system.width;
			canvas.height = ig.system.height;
			var ctx = canvas.getContext("2d");
			ctx.fillStyle = this.clearColor;
			ctx.fillRect(0,0,canvas.width,canvas.height);

			var sprite = new PIXI.Sprite(PIXI.Texture.fromCanvas(canvas));
			ig.system.stage.addChild(sprite);
		}
	},
	...
	draw: function() {
		ig.system.renderer.render(ig.system.stage);
	}
});
...
ig.Loader.inject({
	...
	init: function( gameClass, resources ) {
		...
		for (var i = 0; i < resources.length; i++) {
			if(typeof(resources[i].currentClip) != "undefined") {
				this.audioAssets.push(resources[i]);
				this.audioUnloaded.push(resources[i].path);
			} else {
				this.assets.push(resources[i].path);
			}
		};
	...
	initStage: function() {
		...
		this.symbol = new PIXI.Sprite(PIXI.Texture.fromCanvas(canvas));
		this.symbol.anchor.x = this.symbol.anchor.y = 0.5;
		this.symbol.position.x = ig.system.width/2;
	...
	progress: function() {
		this._loaded++;
		this.percent = Math.round(this._loaded / this.assets.length * 100);
	...
	complete: function() {
		...
		else setTimeout(this.end.bind(this), this.timeout);
	},
	...
	audioLoaded: function(path, status) {
		...
		if(this.audioUnloaded.length == 0) setTimeout(this.end.bind(this), this.timeout);
	},
